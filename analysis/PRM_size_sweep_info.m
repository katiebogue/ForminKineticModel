...
% PRM_SIZE_SWEEP_INFO reads in kpoly_sweep_table and creates gscatter plot
% for each FH1 length and id, analysing the plot and adding information to
% an output table
%
%   out_tab= PRM_SIZE_SWEEP_INFO(kpoly_sweep_table,resultspath)
% 
% Creates and saves scatterplot of PRM size vs. Kpoly ratio, grouped by PRM
% location.
% 
% Looks at the plot data and pulls the following values: 
%   max_val - max kpoly ratio
%   min_val - min kpoly ratio
%   max_x - PRM size for the max kpoly ratio
%   max_x_boundry - PRM size corresponding to 0.05 standard deviations from
%       the max kpoly ratio
%   min_x - PRM size for the min kpoly ratio
%   min_x_boundry - PRM size corresponding to 0.05 standard deviations from
%       the min kpoly ratio
%   
% See also DISTINGUISHABLE_COLORS, GSCATTER, EXPORTGRAPHICS,
% GENERATE_SWEEP_DATA.
