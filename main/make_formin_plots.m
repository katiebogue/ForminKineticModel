...
uit = uitable(fig,'Units','Normalized','Position',[0 0 1 1],'ColumnWidth','auto','ColumnName',{'k_poly Single (log2)', 'k_poly Double (log2)', 'k_poly N-Dimer (log2)','log_2(kpoly ratio)','# Binding Sites','FH1 length', 'Mean PRM size','Mean PRM size x # Binding Sites','Single Capture Rate (log2)','Double Capture Rate (log2)','Dimer Capture Rate (log2)','Single Delivery Rate (log2)','Double Delivery Rate (log2)','Dimer Delivery Rate (log2)','Profilin-Actin Concentration|μM|c_PA','Capture rate constant|μM^(-1)s^(-1)|k_paf','Delivery rate constant|μM^(-1)s^(-1)|k_pab','Reverse Capture Rate|s^(-1)|k_paf_rev'},'Data',[all_kpoly1_nobind, all_kpoly2_nobind, all_kpoly3_nobind, all_log_kpoly3_2_nobind, all_iSite_tot, all_fh1_length, all_mean_PP_length, all_PP_length_x_PP_isite_tot, all_kcap1_nobind, all_kcap2_nobind, all_kcap3_nobind,all_kdel1_nobind,all_kdel2_nobind,all_kdel3_nobind,ones(length(all_fh1_names_nobind),1).*c_PA,ones(length(all_fh1_names_nobind),1).*k_paf,ones(length(all_fh1_names_nobind),1).*k_pab,ones(length(all_fh1_names_nobind),1).*k_paf_rev]);
