...
%PRM Contains information and functions about one specific PRM in a formin.
    ...
    %
    %   See also FORMIN, OPTIONS, EXPERIMENT.
    ...
    methods
        ...
        function out=prval(obj)
            % PRVAL calculate pr value for PRM using equation rather than
            % lookup table
            %
            %   out= PRM.PRVAL calculate pr value for PRM using equation rather than
            %       lookup table
            %
            %   Calculations use the parent formin's fh1length, del_x, and
            %   del_y values, as well as the PRM's dist_FH2
            %   
            %   Output is a FilType with the probability density values for
